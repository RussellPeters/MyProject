...
import com.hcm.camera.video.stream.rtp.AbstractPacketizer;
...
public abstract class MediaStream implements Stream {
	...
	//public static final byte MODE_MEDIARECORDER_API = 0x06;
	
	public static final byte MODE_MY_MEDACODE_API = 0x08;

	public static final byte MODE_MY_X264 = 0x09;

	public static final byte MODE_MY_JPEG = 0x10;
	...
	protected static byte sSuggestedMode = MODE_MY_MEDACODE_API;
	...
	static {
		...
		try {
			...
			sSuggestedMode = MODE_MY_MEDACODE_API;
			sSuggestedMode = MODE_MY_JPEG;
			Log.i(TAG, "Phone supports the MediaCoded API");
		} catch (ClassNotFoundException e) {
			sSuggestedMode = MODE_MY_JPEG; // MODE_MEDIARECORDER_API;
	...
	public void setVideoModel(byte model) {
		mRequestedMode = model;
		mMode = model;
	}
...
//	}
	...
	public synchronized void start() throws IllegalStateException, IOException {
		...
		Log.e("??", sSuggestedMode + "_" + mMode);
		if (mMode == MODE_MY_MEDACODE_API) {
			encodeWithMediaCodec();
		} else if (mMode == MODE_MY_X264) {
			encodeWithX264();
		} else if (mMode == MODE_MY_JPEG) {
			encodeWithJPEG();
		}
	}
	...
	protected abstract void encodeWithX264() throws IOException;

	protected abstract void encodeWithJPEG() throws IOException;
