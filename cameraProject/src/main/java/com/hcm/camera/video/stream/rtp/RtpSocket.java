...
public class RtpSocket implements Runnable {
	...
	public static final int RTP_HEADER_LENGTH = 16;// 12;
	public static final int MTU = 22000;
	// DatagramSocket
	...
	public RtpSocket() {
		...
		mCacheSize = 00;
		mBufferCount = 10; // TODO: reajust that when the FIFO is full
		...
		try {
			...
			while (mBufferCommitted.tryAcquire(4, TimeUnit.SECONDS)) {
				...
				if (mCount++ > 30) {
					...
					NetServer.getIntance().getSocket().sendMediaPack(mPackets[mBufferOut].getData(), mPackets[mBufferOut].getLength());
					// mSocket.send(mPackets[mBufferOut]);
				}
